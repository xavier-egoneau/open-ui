...
Ce skill guide l'agent opencode dans Open UI comme gardien prudent du design system, pas comme simple exécutant rapide.
...
- `rgaa` : revue RGAA generaliste pour composants, pages, formulaires, navigation et interactions.
- `design-sketching` : divergence créative en mode esquisse.
- `design-critique` : relecture UI/UX et diagnostic visuel.
- `design-memory` : maintien de `MEMORY.md` et promotion des apprentissages.
...
Les commandes invocables sont dans `.opencode/commands/` avec le préfixe `open-ui-` pour éviter les collisions. Les fichiers dans `core/` servent de références internes pour garder les prompts alignés avec le workflow projet. Ils doivent agir réellement : lire, modifier si demandé, tester et résumer.
