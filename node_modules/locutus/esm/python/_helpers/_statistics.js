export function assertStatisticsArray(data, functionName) {
    if (!Array.isArray(data)) {
        throw new TypeError(`${functionName}() data must be an array`);
    }
    return data;
}
export function toNumericSequence(data, functionName) {
...
export function toStatisticNumber(value, functionName) {
...
export function sortStatisticsValues(data, functionName) {
    const values = assertStatisticsArray(data, functionName);
    if (values.length === 0) {
        return [];
    }
...
export function statisticsMeanFromSequence(sequence, preferFloat = false) {
    return divideStatisticsValue(sumNumbers(sequence.values), sequence.values.length, sequence.integral, preferFloat);
}
export function statisticsVarianceFromSequence(sequence, divisor, functionName, center) {
...
export function sumProducts(left, right) {
    let total = 0;
...
export function isIntegralStatisticValue(value) {
    return (typeof value === 'boolean' || typeof value === 'bigint' || (typeof value === 'number' && Number.isInteger(value)));
}
export function toFloatStatisticNumber(value, functionName) {
    if (typeof value === 'number') {
        return value;
    }
    if (typeof value === 'boolean') {
