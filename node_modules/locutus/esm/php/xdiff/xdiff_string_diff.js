export function xdiff_string_diff(oldData, newData, contextLines, _minimal) {
    ...
    const _verifyType = function (type, ...values) {
    ...
    const _areTypeOf = function (type, ...values) {
        const argsLen = values.length;
        const basicTypes = ['number', 'boolean', 'string', 'function', 'object', 'undefined'];
        let basicType;
        let i = 0;
    ...
    const regularizeLeadingContext = function (contextBuffer) {
    ...
    const regularizeTrailingContext = function (contextBuffer) {
        if (contextBuffer.length === 0 || context === 0) {
            return [];
        }
