...
exports.toCalendarInteger = toCalendarInteger;
exports.normalizeCalendarYear = normalizeCalendarYear;
exports.normalizeCalendarMonth = normalizeCalendarMonth;
exports.normalizeCalendarDay = normalizeCalendarDay;
exports.normalizeWeekWidth = normalizeWeekWidth;
exports.normalizeFormatColumnWidth = normalizeFormatColumnWidth;
exports.normalizeFormatSpacing = normalizeFormatSpacing;
exports.normalizeMonthWidth = normalizeMonthWidth;
exports.normalizeLineSpacing = normalizeLineSpacing;
exports.normalizeColumnSpacing = normalizeColumnSpacing;
exports.normalizeMonthsPerRow = normalizeMonthsPerRow;
exports.normalizeTimeTuple = normalizeTimeTuple;
exports.isLeapYear = isLeapYear;
exports.daysInMonth = daysInMonth;
exports.pythonWeekday = pythonWeekday;
exports.countLeapYearsBetween = countLeapYearsBetween;
exports.buildWeekHeader = buildWeekHeader;
exports.buildMonthCalendar = buildMonthCalendar;
exports.formatWeek = formatWeek;
exports.formatString = formatString;
exports.formatMonth = formatMonth;
exports.formatYear = formatYear;
exports.timeTupleToUnixSeconds = timeTupleToUnixSeconds;
...
function normalizeCalendarDay(year, month, value, functionName) {
...
function normalizeFormatColumnWidth(value, functionName) {
    return toCalendarInteger(value, functionName);
}
function normalizeFormatSpacing(value, functionName) {
    return toCalendarInteger(value, functionName);
}
function normalizeMonthWidth(value, functionName) {
    return Math.max(toCalendarInteger(value, functionName), 2);
}
function normalizeLineSpacing(value, functionName) {
    return Math.max(toCalendarInteger(value, functionName), 1);
}
function normalizeColumnSpacing(value, functionName) {
    return Math.max(toCalendarInteger(value, functionName), 2);
}
function normalizeMonthsPerRow(value, functionName) {
    return Math.max(toCalendarInteger(value, functionName), 1);
}
...
function countLeapYearsBetween(startYear, endYear) {
...
function formatMonth(year, month, width, lineSpacing) {
...
function formatYear(year, width, lineSpacing, columnSpacing, monthsPerRow) {
    const monthWidth = 7 * (width + 1) - 1;
    const yearWidth = monthWidth * monthsPerRow + columnSpacing * (monthsPerRow - 1);
    const lines = [centerText(String(year), yearWidth).replace(/\s+$/, ''), '\n'.repeat(lineSpacing)];
    const header = buildWeekHeader(width);
    const monthRows = Array.from({ length: Math.ceil(12 / monthsPerRow) }, (_, rowIndex) => Array.from({ length: monthsPerRow }, (_, columnIndex) => rowIndex * monthsPerRow + columnIndex + 1).filter((month) => month <= 12));
    for (const months of monthRows) {
        lines.push('\n'.repeat(lineSpacing));
        lines.push(formatString(months.map((month) => centerText(MONTH_NAMES[month] ?? '', monthWidth)), monthWidth, columnSpacing).replace(/\s+$/, ''));
        lines.push('\n'.repeat(lineSpacing));
        lines.push(formatString(months.map(() => header), monthWidth, columnSpacing).replace(/\s+$/, ''));
        lines.push('\n'.repeat(lineSpacing));
        ...
        for (let weekIndex = 0; weekIndex < height; weekIndex += 1) {
            lines.push(formatString(weeks.map((calendarRows) => weekIndex >= calendarRows.length
                ? ''
                : formatWeek((calendarRows[weekIndex] ?? []).map((day, weekday) => [day, weekday]), width)), monthWidth, columnSpacing).replace(/\s+$/, ''));
            lines.push('\n'.repeat(lineSpacing));
        }
    }
    return lines.join('');
}
function countLeapYearsBefore(yearExclusive) {
...
function joinCalendarLines(lines, lineSpacing) {
    return `${lines.map((line) => line.replace(/\s+$/, '')).join('\n'.repeat(lineSpacing))}\n`;
}
function formatMonthWeeks(year, month, width) {
    return buildMonthCalendar(year, month).map((week) => formatWeek(week.map((day, weekday) => [day, weekday]), width));
}
