...
exports.ParseInLocation = ParseInLocation;
...
const isTokenAt = (layout, index) => GO_PARSE_TOKENS.find((token) => layout.startsWith(token, index));
...
const resolveTwoDigitYear = (year) => (year >= 69 ? 1900 + year : 2000 + year);
const createUtcDate = (year, month, day, hour, minute, second) => {
    const date = new Date(0);
    date.setUTCFullYear(year, month - 1, day);
    date.setUTCHours(hour, minute, second, 0);
    return date;
};
const parseGoTime = (layout, value) => {
...
const assertValidLocation = (location) => {
    try {
...
const getOffsetMinutesAt = (timeZone, utcMillis) => {
...
const resolveWallTimeInLocation = (timeZone, localMillis) => {
...
function ParseInLocation(layout, value, location) {
