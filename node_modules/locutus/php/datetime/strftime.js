...
const isLcTime = (value) => isStringArray(value, 'a') &&
...
function strftime(fmt, timestamp) {
    ...
    const lcTimeBag = (0, _phpRuntimeState_ts_1.getPhpLocaleGroup)('LC_TIME', 'LC_TIME');
    if (!lcTimeBag || !isLcTime(lcTimeBag)) {
        return '';
    }
    const _xPad = function (x, pad, r = 10) {
    ...
    while (fmt.match(/%[cDFhnrRtTxX]/)) {
        fmt = fmt.replace(/%([cDFhnrRtTxX])/g, function (_m0, m1) {
    ...
    const str = fmt.replace(/%([aAbBCdegGHIjklmMpPsSuUVwWyYzZ%])/g, function (_m0, m1) {
        const f = formats[m1];
        if (typeof f === 'string') {
