...
exports.money_format = money_format;
const _phpRuntimeState_ts_1 = require("../_helpers/_phpRuntimeState.js");
const setlocale_ts_1 = require("../strings/setlocale.js");
const toStringField = (value) => (typeof value === 'string' ? value : '');
const toNumberField = (value) => (typeof value === 'number' ? value : 0);
const toNumberArray = (value) => Array.isArray(value) ? value.filter((item) => typeof item === 'number') : [];
const toMonetaryLocale = (value) => {
    return {
...
function money_format(format, number) {
    ...
    const monetaryGroup = (0, _phpRuntimeState_ts_1.getPhpLocaleGroup)('LC_MONETARY', 'LC_MONETARY');
    if (!monetaryGroup) {
        return null;
    }
    const monetary = toMonetaryLocale(monetaryGroup);
    const doReplace = function (_n0, flags = '', _n2, width = '', _n4, left = '', _n6, right = '', conversion) {
        let value = '';
        let repl = '';
        if (conversion === '%') {
        ...
        if (right === '0') {
            ...
            if (rightNum === 0) {
                ...
                const rounded = Math.round(parseFloat(fraction.slice(0, rightNum) + '.' + fraction.substring(rightNum, rightNum + 1)));
